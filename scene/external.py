...
def MutiPlane_anchor_init(monodepth, xyz, view_camera, confidence_map=None,  # ✅ 新增参数
                          plane_num=4, sample_size=8, muti_mode="neighbor", 
                          itera_num=1, feat_dim=32, conf_thresh=0.6, soft=False):
    """
    基于多平面的 Anchor 初始化（加入置信度软筛选）
    """
    ...
    if confidence_map is not None:
        conf_values = confidence_map[sampler_axis[:, 1].long(), sampler_axis[:, 0].long()]
        if soft:
            # --- 软筛选：用置信度作为权重 ---
            weight = torch.clamp(conf_values, min=conf_thresh)
            weight = weight / weight.max()
        else:
            # --- 硬筛选：直接剔除低置信度点 ---
            mask = conf_values > conf_thresh
            num_true = mask.sum().item()           # True 的个数
            num_false = mask.numel() - num_true    # False 的个数
            sampler_axis = sampler_axis[mask]
            sampler_label = sampler_label[mask]
            weight = torch.ones_like(sampler_label, dtype=torch.float32, device="cuda")
    else:
        weight = torch.ones(sampler_label.shape[0], device="cuda")
