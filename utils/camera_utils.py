...
def loadCam(args, id, cam_info, resolution_scale):
    ...
    fx_new = fx_new * scale_x
    fy_new = fy_new * scale_y
    cx_new = cx_new * scale_x
    cy_new = cy_new * scale_y

    # ------------------------
    # 缩放深度图
    # ------------------------
    depth_resized = None
    depth_interp='bilinear'
    if hasattr(cam_info, 'depth') and cam_info.depth is not None:
        depth = torch.tensor(cam_info.depth, dtype=torch.float32).unsqueeze(0).unsqueeze(0)  # [1,1,H,W]
        # 注意这里的目标尺寸是 (height, width)
